...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = StructField(columnNames.head, StringType, false) ::
      columnNames.tail.map( (name: String) => StructField(name, DoubleType, false ))
    StructType(fields)
  }
  ...
  def row(line: List[String]): Row =
    return Row.fromSeq(line.head :: line.tail.map( _.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val (primary, temp) = columnNames.partition(name =>
      name.startsWith("t01") ||
      name.startsWith("t03") ||
      name.startsWith("t11") ||
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val aggregated = grouped.agg(
      ...
      TimeUsageRow(working, sex, age, primary.round, work.round, other.round)
    }
    aggregated.orderBy($"working", $"sex", $"age")
  }
}
